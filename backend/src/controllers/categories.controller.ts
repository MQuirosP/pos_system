...
export class CategoryController {
    ...
    createCategory(req: Request, res: Response, next: NextFunction) {
        this.handleControllerOperation(req, res, next, async () => {
            ...
            return res.success(
                newCategory, "Category created successfully.", 201
            );
        });
    }
    ...
    getCategories(req: Request, res: Response, next: NextFunction) {
        this.handleControllerOperation(req, res, next, async () => {
            ...
            const responseMessage = categories.length === 0 ? "No categories found." : "All categories fetched successfully.";
    ...
    updateCategory(req: Request, res: Response, next: NextFunction) {
        this.handleControllerOperation(req, res, next, async () => {
            ...
            const updatedCategory = await this.categoryService.updateCategory(catId, categoryData);
    ...
    deleteCategory(req: Request, res: Response, next: NextFunction) {
        this.handleControllerOperation(req, res, next, async () => {
            const catId = parseInt(req.params.id);
            const categoryToDelete = await this.categoryService.fetchCategoryByPK(catId);
            await this.categoryService.deleteCategory(catId);

            return res.success(categoryToDelete, "Category deleted successfully.", 200);
        });
    }
}